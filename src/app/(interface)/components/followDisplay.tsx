...
function FollowDisplay(props: {
  ...
  const [followers, setFollowers] = useState(new Array());
  const [following, setFollowing] = useState(new Array());
  ...
  return (
    <div>
      <p>
        <a role="button" onClick={() => handleShow("Followers")}>
          <b>{props.followersIds.length}</b> Followers
        </a>{" "}
        |
        <a role="button" onClick={() => handleShow("Following")}>
          <b> {props.followingIds.length}</b> Following
        </a>
      </p>
